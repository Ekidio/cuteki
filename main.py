...
class AudioWaveformThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            target_points = 12000
            block_size = max(1, int(np.ceil(len(samples) / target_points)))
            block_count = int(np.ceil(len(samples) / block_size))
            ...
            blocks = padded_samples.reshape(block_count, block_size)
            reduced_samples = np.column_stack((blocks.min(axis=1), blocks.max(axis=1))).ravel()
...
class WaveformTimeAxis(pg.AxisItem):
    """Top timeline with readable time labels in mm:ss format."""

    def tickStrings(self, values, scale, spacing):
        labels = []
        for value in values:
            total_seconds = max(0, value / 1000.0)
...
class FullVideoCutterGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.plot_widget = InteractivePlotWidget(
            axisItems={'top': WaveformTimeAxis(orientation='top')}
        )
    ...
    def draw_waveform(self, samples, duration_sec):
        ...
        display_samples = np.clip(samples * 1.6, -1.0, 1.0)
        self.plot_widget.plot(
            x,
            display_samples,
            pen=pg.mkPen('#30d158', width=2),
            fillLevel=0,
            brush=pg.mkBrush(48, 209, 88, 80)
        )
